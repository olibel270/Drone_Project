...
class Orientation3D(object):
    ...
    def publishSensorData(self, sensor_data, calibration_status):
        tmp = EulerAngles()
        self.pozyx.getEulerAngles_deg(tmp)
        phi = -(tmp.roll)*np.pi/180
        theta = (-(tmp.pitch)-180)*np.pi/180
        if theta<-np.pi:
            theta+=2*np.pi #wrap around
        psi = (tmp.heading)*np.pi/180
...
def pozyx_positioning_pub(r,o):
    mavlink_pub = rospy.Publisher('/mavlink/to', Mavlink, queue_size=100)
    ...
    while not rospy.is_shutdown():
        ...
        attitude = pypozyx.EulerAngles()
        pozyx.doPositioning(coords, remote_id=remote_id)
        pozyx.getEulerAngles_deg(attitude)
        pose = types.SimpleNamespace()

        pose.phi = -(attitude.roll)*np.pi/180
        pose.theta = (-(attitude.pitch)-180)*np.pi/180
        if pose.theta<-np.pi:
            pose.theta+=2*np.pi #wrap around
        pose.psi = (float(attitude.heading)-180)*np.pi/180
        #if pose.psi>180:
         #   pose.psi = -(360-pose.psi)
